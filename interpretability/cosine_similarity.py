...
def main(args):
    ...
    # print(xs.reshape(args.p, args.p, 2)[:, 1])
    ...
    for layer_idx, (q, k, v) in enumerate(qkv_list):
        ...
        for head_idx in range(args.n_head):
            ...
            ov_out_for_plot = ov_out[inv_perm, args.dim*end_pair_id:args.dim*(end_pair_id + 1), :].view(args.p, args.p, args.dim, -1) # All pairs with shot end_pair, and pos shift for given head. (p, p, n_embd)
            ov_out_for_plot /= ov_out_for_plot.norm(dim=-1, keepdim=True)
            mean = ov_out_for_plot.mean(dim=-1, keepdim=True)
            std = ov_out_for_plot.std(dim=-1, keepdim=True, unbiased=True)
            # Add small epsilon to avoid division by zero
            eps = 1e-8
            ov_out_for_plot = (ov_out_for_plot - mean) / (std + eps)
            ov_out_for_plot /= ov_out_for_plot.norm(dim=-1, keepdim=True)
            
            for plot_key in ['x', 'y', 'z']:
                
                match plot_key:
                    case 'x':
                        fig, ax = plt.subplots(1, 1, figsize=(7, 6), constrained_layout=True)
                        cosine_similarity = ov_out_for_plot[:, 1, 0, :] @ ov_out_for_plot[:, 1, 0, :].T
                    case 'y':
                        fig, ax = plt.subplots(1, 1, figsize=(7, 6), constrained_layout=True)
                        cosine_similarity = ov_out_for_plot[:, :, 1, :].view(args.p**2, -1) @ ov_out_for_plot[:, :, 1, :].view(args.p**2, -1).T
                    case 'z':
                        fig, ax = plt.subplots(1, 1, figsize=(7, 6), constrained_layout=True)
                        cosine_similarity = ov_out_for_plot[:, :, 2, :].view(args.p**2, -1) @ ov_out_for_plot[:, :, 2, :].view(args.p**2, -1).T
                
                sns.heatmap(cosine_similarity, 
                            annot=False,
                            cmap='coolwarm', 
                            vmin=-1.0, 
                            vmax=1.0,
                ...
                ax.invert_yaxis()
                ax.tick_params(axis='both', which='major', length=5, width=0.5, reset=True)
                ax.tick_params(bottom=True, top=False, left=True, right=False)
                
                tick_positions = np.arange(0, len(cosine_similarity), args.p) if plot_key != 'x' else np.arange(0, len(cosine_similarity), 5)
                # Set ticks and labels
                ax.set_xticks(tick_positions + 0.5)
                ax.set_yticks(tick_positions + 0.5)
                ax.set_xticklabels(tick_positions, fontsize=6 if plot_key != 'x' else 12, rotation=0, va='center')
                ax.set_yticklabels(tick_positions, fontsize=6 if plot_key != 'x' else 12, rotation=45, ha='center')
                
                # Optionally add gridlines at the tick positions
                ax.grid(False)
                ...
                if args.savefig is True:
                    fig_path = f'./cosine_similarity/{plot_key}_shots{end_pair_id}_layer{layer_idx}_head{head_idx}_Ws{Ws[args.task_id][0]}_{Ws[args.task_id][1]}_{args.model_name}_p{args.p}_base{args.base}_row{args.n_point_per_row}_ntask{args.n_tasks}_nvar{args.n_var}_dsplit{args.split_data}_dfrac{args.data_pct:.1f}_{args.act_name}_n{args.n_embd}_h{args.n_head}_d{args.n_layer}_lctx{args.block_size}_I{args.seed}_dI{args.data_seed}_{args.optim}_bs{args.bs}_T{args.steps:d}_Tw{args.warmup_steps:d}_Trshf{args.reshuffle_step}_lr{args.lr:0.2e}_wd{args.wd:.2e}.pdf'
    ...
    for layer_idx, output in enumerate(output_list):
        ...
        out_for_plot = output[inv_perm, args.dim*end_pair_id:args.dim*(end_pair_id + 1), :].view(args.p, args.p, args.dim, -1).cpu() # All pairs with shot end_pair, and pos shift for given head. (p, p, n_embd)
    
        mean = out_for_plot.mean(dim=-1, keepdim=True)
        std = out_for_plot.std(dim=-1, keepdim=True, unbiased=True)
        # Add small epsilon to avoid division by zero
        eps = 1e-8
        out_for_plot = (out_for_plot - mean) / (std + eps)
        out_for_plot /= out_for_plot.norm(dim=-1, keepdim=True)
        
        for plot_key in ['x', 'y', 'z']:    
            match plot_key:
                case 'x':
                    fig, ax = plt.subplots(1, 1, figsize=(6, 6), constrained_layout=True)
                    cosine_similarity = out_for_plot[:, 1, 0, :] @ out_for_plot[:, 1, 0, :].T
                case 'y':
                    fig, ax = plt.subplots(1, 1, figsize=(6, 6), constrained_layout=True)
                    cosine_similarity = out_for_plot[:, :, 1, :].view(args.p**2, -1) @ out_for_plot[:, :, 1, :].view(args.p**2, -1).T
                case 'z':
                    fig, ax = plt.subplots(1, 1, figsize=(6, 6), constrained_layout=True)
                    cosine_similarity = out_for_plot[:, :, 2, :].view(args.p**2, -1) @ out_for_plot[:, :, 2, :].view(args.p**2, -1).T
            ...
            if args.savefig is True:
                fig_path = f'./cosine_similarity/{plot_key}_block_shots{end_pair_id}_layer{layer_idx}_Ws{Ws[args.task_id][0]}_{Ws[args.task_id][1]}_{args.model_name}_p{args.p}_base{args.base}_row{args.n_point_per_row}_ntask{args.n_tasks}_nvar{args.n_var}_dsplit{args.split_data}_dfrac{args.data_pct:.1f}_{args.act_name}_n{args.n_embd}_h{args.n_head}_d{args.n_layer}_lctx{args.block_size}_I{args.seed}_dI{args.data_seed}_{args.optim}_bs{args.bs}_T{args.steps:d}_Tw{args.warmup_steps:d}_Trshf{args.reshuffle_step}_lr{args.lr:0.2e}_wd{args.wd:.2e}.pdf'
